...
STEP_USER_DATA_SCHEMA = vol.Schema(
    vol.Union(
        ...
        vol.Or(
            {
                vol.IsTrue(CONF_USE_ENTRA_ID_AUTH): bool,
                vol.Required(CONF_TENANT_ID): str,
                vol.Required(CONF_CLIENT_ID): str,
                vol.Required(CONF_CLIENT_SECRET): str,
            },
            {
                vol.IsFalse(CONF_USE_ENTRA_ID_AUTH): bool,
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> None:
    ...
    """
    if data.get(CONF_USE_ENTRA_ID_AUTH, False):
        ...
        token_provider = data[CONF_API_KEY]
...
class AzureOpenAISubentryFlowHandler(ConfigSubentryFlow):
    """Flow for managing Azure OpenAI subentries."""
    ...
    async def _get_location_data(self) -> dict[str, str]:
        ...
        if zone_home is not None:
            if self._get_entry().data.get(CONF_USE_ENTRA_ID_AUTH, False):
                from azure.identity.aio import ClientSecretCredential, get_bearer_token_provider

                credential = ClientSecretCredential(
                    tenant_id=self._get_entry().data[CONF_TENANT_ID],
                    client_id=self._get_entry().data[CONF_CLIENT_ID],
                    client_secret=self._get_entry().data[CONF_CLIENT_SECRET],
                )

                token_provider = get_bearer_token_provider(credential, "https://cognitiveservices.azure.com/.default")
            else:
                token_provider = self._get_entry().data[CONF_API_KEY]

            client = openai.AsyncAzureOpenAI(
                api_key=token_provider,
                base_url=normalize_azure_endpoint(self._get_entry().data[CONF_API_BASE]),
                api_version="latest",
